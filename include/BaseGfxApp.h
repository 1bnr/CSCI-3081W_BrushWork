...
class BaseGfxApp {
 public:
    ...
    // Call this function to start the program.  It will not return until the
    // graphics window is closed.
    void runMainLoop();

    // Subclasses should override this method in order to draw graphics to the
    // screen.
    virtual void display() {}

    // This is a convenience function that draws an array of pixel data to the
    // screen.
    void drawPixels(
        int start_x,
        int start_y,
    ...
    // The following functions provide callbacks for user interface events in
    // the GLUT window.
    ...
    // This is the callback that tells you when the user has interacted with a
    // GLUI widget
    virtual void gluiControl(int controlID) {}


    // Glut calls this when the user resizes the graphics window by dragging on
    // it with the mouse. The default implementation in BaseGfxApp is to ignore
    // this interaction and force the window  to remain the same size.  But, if
    // desired, subclasses can override this method and implement the logic to
    // allow for interactive resizing of windows.
 ...
 protected:
    /* GLUT and GLUI event callbacks are sent to the current window/app. Right
       now, there is only one window anyway (not counting the GLUI UI
       window.. in the future could be extended to support more windows.  In any
       case, some structure like this is always needed when using glut with C++,
       since the glut callbacks must be either global or static functions.
    */
