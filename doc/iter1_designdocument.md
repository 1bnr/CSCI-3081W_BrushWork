...
The design choice works because it doesn't change the fact that eraser is still a tool.  Our design allows eraser to function the same way as every other tool, it just has the extra requirement of what color it draws with.  Maintaining this fact was what our group saw as most important when considering the eraser.  Our code abides by this, and this is the backing behind our implementation.  Another option would have been to create eraser in its own sub-class within Tool.  Our group didn't do this because we saw that extra step as unnecessary.  Our idea was to keep relationships as simple as possible, and separating eraser out further from the other tools would not have achieved that.
